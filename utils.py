...
def get_href(soup: object) -> list:
    """
    Given soup object, return href list
    """
    div_tag = soup.find(
        "div",
        {"class": "wp-block-query alignwide has-global-padding is-layout-constrained"},
    )
    a_tag = div_tag.find_all("a", href=True)
    href_list = [x["href"] for x in a_tag if "page" not in x["href"]]
